 ...
 * @since 1.0.0
 */
public class CancelTicketCommand implements PluginCommandExecutor {
    @Override
    public boolean onCommand(@NotNull PluginCommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        if (ticket == null) {
            player.sendMessage(Component.text("§cDu hast kein Ticket erstellt."), Prefix.TICKET);
            return false;
        }
        ticketService.removeTicket(ticket);
        player.sendMessage(Component.text("Du hast dein Ticket gelöscht."), Prefix.TICKET);
        return false;
    }
}
